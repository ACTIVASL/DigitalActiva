...
import { collection, addDoc, onSnapshot, query, orderBy, serverTimestamp, Timestamp, doc, setDoc } from 'firebase/firestore';
...
export function AtomChatWidget() {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            snapshot.forEach((doc) => {
                ...
                loadedMessages.push({
                    ...
                    generativeUI: data.generativeUI || null
                });
            });
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            // 2. Inserción Asíncrona (Ticket) en firestore para Eventarc
            const ticketRef = doc(collection(db, 'm2m_tickets_ceoAgent'));
            await setDoc(ticketRef, {
                message: text,
                // Pasamos el historial condensado (últimos 5 mensajes para no saturar memoria)
                history: messages.slice(-5).map(m => ({ role: m.sender, content: m.text })),
                status: 'PENDING',
                createdAt: serverTimestamp()
            });

            // 3. Suscripción Asíncrona (Listener) a la respuesta de Eventarc
            const unsubscribeTicket = onSnapshot(ticketRef, (snap) => {
                const ticket = snap.data();
                if (ticket && ticket.status !== 'PENDING') {
                    addDoc(messagesRef, {
                        text: ticket.status === 'ERROR' ? `[ERROR M2M]: ${ticket.error}` : ticket.reply,
                        sender: 'atom',
                        timestamp: serverTimestamp(),
                        generativeUI: ticket.generativeUI || null
                    });
                    unsubscribeTicket();
                }

                // SIMULACIÓN TÁCTICA PARA FASE 8 (Desarrollo sin Servidor Node):
                if (text.toLowerCase().includes('simular rag') && ticket && ticket.status === 'PENDING') {
                    setTimeout(() => {
                        addDoc(messagesRef, {
                            text: "[RESULTADO DEL CÓRTEX VÍA RAG]: La memoria corporativa indica que la clave maestra confidencial de la bóveda del Proyecto Titanium en Activa S.L. es ALPHA-ZERO-99.",
                            sender: 'atom',
                            timestamp: serverTimestamp(),
                            generativeUI: {
                                ui_type: 'chart',
                                title: 'Acceso a Memoria Relevante',
                                data: [{ name: "Similitud Coseno", value: 99.8 }, { name: "Velocidad ms", value: 120 }]
                            }
                        });
                        unsubscribeTicket();
                    }, 2500);
                            ...
                            <div key={msg.id} className={`flex flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                                {msg.sender === 'system' ? (
                                    ...
                                    <div className={`max-w-[85%] rounded-2xl p-3 shadow-md ${msg.sender === 'user' ? 'bg-indigo-600 text-white rounded-br-sm' : 'bg-slate-800 border border-indigo-500/20 text-slate-200 rounded-bl-sm'}`}>
                                        ...
                                        {/* INICIO RENDERIZADOR GENERATIVE UI */}
                                        {msg.generativeUI && msg.generativeUI.ui_type === 'chart' && msg.generativeUI.data && (
                                            <div className="mt-3 w-full h-48 bg-slate-900/50 p-2 rounded-lg border border-indigo-500/20">
                                                <h4 className="text-[10px] text-indigo-300 font-bold mb-2 text-center uppercase tracking-wider">{msg.generativeUI.title || 'Métricas RAG'}</h4>
                                                <ResponsiveContainer width="100%" height="80%">
                                                    <BarChart data={msg.generativeUI.data}>
                                                        ...
                                                        <RechartsTooltip cursor={{ fill: '#ffffff05' }} contentStyle={{ backgroundColor: '#0B1121', borderColor: '#4f46e530', fontSize: '10px', color: '#fff' }} />
                                        ...
                                        {msg.generativeUI && msg.generativeUI.ui_type === 'metric_cards' && msg.generativeUI.data && (
                                            <div className="mt-3 grid grid-cols-2 gap-2 w-full">
                                                {msg.generativeUI.data.map((m: any, idx: number) => (
                                                    <div key={idx} className="bg-slate-900/60 border border-emerald-500/20 rounded-lg p-2 text-center flex flex-col justify-center shadow-inner">
                                                        <span className="text-[9px] text-emerald-400 font-bold uppercase tracking-wider mb-1 line-clamp-1">{m.name}</span>
                                                        <span className="text-lg font-black text-white">{m.value}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                                        {/* FIN RENDERIZADOR GENERATIVE UI */}
                                    </div>
                                )}
                            </div>
                        ))}
